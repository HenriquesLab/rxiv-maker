...
def convert_figure_references_to_latex(text: MarkdownContent) -> LatexContent:
    ...
    # Convert @sfig:id followed by space and panel letter to Suppl. Fig. \ref{sfig:id}PanelLetter (no space)
    # Use empty group {} to prevent LaTeX from inserting unwanted spaces after \ref{}
    text = re.sub(r"@sfig:([a-zA-Z0-9_-]+)\s+([a-zA-Z])\b", r"Suppl. Fig. \\ref{sfig:\1}{}\2", text)

    # Convert @sfig:id to Suppl. Fig. \ref{sfig:id} (supplementary figures)
    text = re.sub(r"@sfig:([a-zA-Z0-9_-]+)", r"Suppl. Fig. \\ref{sfig:\1}", text)
