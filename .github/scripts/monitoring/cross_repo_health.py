...
from utils import make_github_request, retry_with_backoff
...
class CrossRepoHealthMonitor:
        ...
        try:
            ...
            if repo_info:
                ...
                if all(check in ['pass', 'warn'] for check in health['checks'].values()):
                    health['status'] = 'healthy' if all(check == 'pass' for check in health['checks'].values()) else 'warning'
                else:
    ...
    def _generate_health_summary(self):
        ...
        failing_repos = sum(1 for health in self.health_status.values() if health['status'] in ['failing', 'error', 'inaccessible'])
...
def main():
    ...
    try:
        ...
        failing_repos = sum(1 for health in health_status.values() 
                          if health['status'] in ['failing', 'error', 'inaccessible'])
